...
const getMentionsForPage = async (slug: string) => {
  ...
  let containerMentions: WebMention[][] = allMentions
    .filter((a) => formattedSlug === a.id)
    .map((a) => a.data);
  console.log('');
  // console.log(mentionsForPage[0])
  ...
  if (containerMentions.length == 0) {
    ...
    data.replies = mentions.filter((a)=>a.wmProperty == enums.wmProperty.enum['in-reply-to'])
    data.likes = mentions.filter((a)=>a.wmProperty == enums.wmProperty.enum['like-of'])
    data.reposts = mentions.filter((a)=>a.wmProperty == enums.wmProperty.enum['repost-of'])
    data.bookmarks = mentions.filter((a)=>a.wmProperty == enums.wmProperty.enum['bookmark-of'])
    data.mentions = mentions.filter((a)=>a.wmProperty == enums.wmProperty.enum['mention-of'])
    data.rsvp = mentions.filter((a)=>a.wmProperty == enums.wmProperty.enum.rsvp)
    return data;
  }
};
...
function formatSlugToID(slug: string) {
  ...
  let formatted = slug.replaceAll('/', '_');
